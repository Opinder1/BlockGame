...
const MonitorSize Monitor::get_size() {
    const GLFWvidmode* mode = glfwGetVideoMode(monitor);

    return { (uint32)mode->width, (uint32)mode->height };
}

const MonitorPos Monitor::get_pos() {
    MonitorPos pos;
...
const std::string MonitorConnectEvent::to_string() const {
...
const std::string MonitorDisconnectEvent::to_string() const {
